...
const AddBannerImagesForm = ({ discoDetailId }: { discoDetailId: string }) => {
            ...
            return (
              <div key={field.id} className="w-full">
                ...
                {errors?.bannerImages?.[index] && (
                  <p className="text-xs italic text-red-500">{String(errors.bannerImages[index]?.root?.message)}</p>
                )}
              </div>
            );
          })}
...
export default AddBannerImagesForm;
